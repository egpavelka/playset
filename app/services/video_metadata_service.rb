class VideoMetadataService
  ...
  # (if present), since t he format is almost always [YYYY] or (YYYY).
...
``    end
  ...
  def parse_lookup_params(arr)
    # Basically check for parentheses and second splitters in the strings
    # created by split_title and, if they exist, try looking up the song
    # first with everything up to the parentheses/split mark, and if
    # that doesn't return anything, try again with the next segment included
    arr.each do |str|
  ...
  # Freddie Gibbs & Madlib - Deeper (Official) - Pinata
  ...
  # Don't split on standard splitting characters if they occur
  ...
  def extraneous_phrases
    phrases = ['1080p', '720p', 'audio', 'audio only', 'clean version', 'explicit', 'full song', 'hd', 'hd video', 'long version', 'lyric video', 'lyrics', 'lyrics on screen', 'new song', 'new video', 'official', 'official audio', 'official hd video', 'official music video', 'official song', 'official video', 'stereo version', 'video', 'videoclip', 'video clip', 'visualization', 'vizualizer', 'with lyrics', 'w/ lyrics', '+ lyric']
    arr = Array[]
    phrases.each { |phrase| arr << Regexp.new(Regexp.quote(phrase.to_s), Regexp::IGNORECASE) }
    arr
  end
end
